...
$Global:psTwitterEndpointFriends		= 'https://api.twitter.com/1.1/friends/list.json'
$Global:psTwitterEndpointFollowers		= 'https://api.twitter.com/1.1/followers/list.json'
...
function Get-TwitterFollowers
{
	...
	PROCESS
	{
		...
		#{			
			...
			$fixedFollowers = $(ConvertTo-HexEscaped -InputText $Followers)
            ...
            While ($cursor -ne 0) {
			    ...
			    $oAuthRequestString = (Connect-OAuthTwitter -followers $fixedFollowers -cursor $cursor)
			
			    (Write-Status -Message "START  - Sending HTTP GET via REST to Twitter" -Status "INFO" -Debugging:$psTwitterDebugging -Logging:$psTwitterLogging -Logpath $psTwitterLogPath)
			
			    # Call the REST API to handle the final OAUTH GET
                # Changed the structure of the URI because GET can't have body.
                
                $URI = "$($psTwitterEndpointFollowers)?screen_name=$($fixedFollowers)&cursor=$($cursor)"
                Write-Warning $URI

			    $Followers += Invoke-RestMethod -URI $URI -Method Get -Headers @{ 'Authorization' = $oAuthRequestString } -ContentType "application/x-www-form-urlencoded" #| Out-Null

    			(Write-Status -Message "FINISH - Sending HTTP GET via REST to Twitter" -Status "INFO" -Debugging:$psTwitterDebugging -Logging:$psTwitterLogging -Logpath $psTwitterLogPath)
	
		        $cursor = [double] $Followers.next_cursor
...
Get-TwitterFollowers mkellerman	