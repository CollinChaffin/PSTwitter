...
$Global:psTwitterEndpointUserLookup		= 'GET&https://api.twitter.com/1.1/users/lookup.json'
$Global:psTwitterEndpointFriendsList	= 'GET&https://api.twitter.com/1.1/friends/list.json'
$Global:psTwitterEndpointFriendsIds		= 'GET&https://api.twitter.com/1.1/friends/ids.json'
$Global:psTwitterEndpointFollowersList	= 'GET&https://api.twitter.com/1.1/followers/list.json'
$Global:psTwitterEndpointFollowersIds	= 'GET&https://api.twitter.com/1.1/followers/ids.json'
...
function Connect-OAuthTwitter
{
	...
	PROCESS
	{
		try
		{
            ...
            $oAuthSignature = (New-TwitterOAuthSignature -objOAuth $objOAuth)
...
function Get-TwitterFriends
{
	...
	BEGIN
	{
		(Write-Status -Message "START  - Get-TwitterFriends function execution" -Status "INFO" -Debugging:$psTwitterDebugging -Logging:$psTwitterLogging -Logpath $psTwitterLogPath)
	}
	PROCESS
	{
		try
		{			
            ...
            $Ids = Get-TwitterFriendsIds $Username

            $m = 100; $i=0; $j=0
            
            while ($i -lt $ids.ids.count) {
                ...
                If ($j -eq $m) {
                    ...
                    $iStart = $i - $m; $iEnd = $i - 1
                    $user_id = $Ids.ids[$iStart..$iEnd]
                    $Friends += Get-TwitterUserLookup ($user_id -join ',')
                    $j=0 
	...
	END
	{
		(Write-Status -Message "FINISH - Get-TwitterFriends function execution" -Status "INFO" -Debugging:$psTwitterDebugging -Logging:$psTwitterLogging -Logpath $psTwitterLogPath)
	}
}

function Get-TwitterFriendsList
{
	<#
	.SYNOPSIS
		Sends a Twitter Tweet
	...
	BEGIN
	{
		(Write-Status -Message "START  - Get-TwitterFriendsList function execution" -Status "INFO" -Debugging:$psTwitterDebugging -Logging:$psTwitterLogging -Logpath $psTwitterLogPath)
	}
	PROCESS
	{
		try
		{			
            ...
            #While ($cursor -ne 0) {
                ...
                $psTwitterMethod = ($psTwitterEndpointFriendsList -Split '&')[0]
                $psTwitterEndpoint = ($psTwitterEndpointFriendsList -Split '&')[1]
			    $psTwitterParameters = "screen_name=$($fixedUsername)"

                # Call our main connect routine to do the oAuth heavy lifting
			    $oAuthRequestString = (Connect-OAuthTwitter $psTwitterEndpointFriendsList $psTwitterParameters )
	...
	END
	{
		(Write-Status -Message "FINISH - Get-TwitterFriendsList function execution" -Status "INFO" -Debugging:$psTwitterDebugging -Logging:$psTwitterLogging -Logpath $psTwitterLogPath)
	}
}

function Get-TwitterFriendsIds
{
	<#
	.SYNOPSIS
		Sends a Twitter Tweet
	...
	BEGIN
	{
		(Write-Status -Message "START  - Get-TwitterFriendsIds function execution" -Status "INFO" -Debugging:$psTwitterDebugging -Logging:$psTwitterLogging -Logpath $psTwitterLogPath)
	}
	PROCESS
	{
		try
		{			
            ...
            #While ($cursor -ne 0) {
                ...
                $psTwitterMethod = ($psTwitterEndpointFriendsIds -Split '&')[0]
                $psTwitterEndpoint = ($psTwitterEndpointFriendsIds -Split '&')[1]
                ...
                # Call our main connect routine to do the oAuth heavy lifting
			    $oAuthRequestString = (Connect-OAuthTwitter $psTwitterEndpointFriendsIds $psTwitterParameters )
	...
	END
	{
		(Write-Status -Message "FINISH - Get-TwitterFriendsIds function execution" -Status "INFO" -Debugging:$psTwitterDebugging -Logging:$psTwitterLogging -Logpath $psTwitterLogPath)
	}
}
...
function Get-TwitterFollowers
{
	...
	PROCESS
	{
		...
		{			
            ...
            $Ids = Get-TwitterFollowersIds $Username
...
function Get-TwitterFollowersList
{
	<#
	.SYNOPSIS
		Sends a Twitter Tweet
	...
	PROCESS
	{
		...
		{			
            ...
            #While ($cursor -ne 0) {
                ...
                # Call our main connect routine to do the oAuth heavy lifting
			    $oAuthRequestString = (Connect-OAuthTwitter $psTwitterEndpointFollowers $psTwitterParameters )
...
function Get-TwitterFollowersIds
{
	<#
	.SYNOPSIS
		Sends a Twitter Tweet
	...
	BEGIN
	{
		(Write-Status -Message "START  - Get-TwitterFollowersIds function execution" -Status "INFO" -Debugging:$psTwitterDebugging -Logging:$psTwitterLogging -Logpath $psTwitterLogPath)
	}
	PROCESS
	{
		try
		{			
            ...
            $psTwitterMethod = ($psTwitterEndpointFollowersIds -Split '&')[0]
            $psTwitterEndpoint = ($psTwitterEndpointFollowersIds -Split '&')[1]
			$psTwitterParameters = "screen_name=$($fixedUsername)"

            # Call our main connect routine to do the oAuth heavy lifting
			$oAuthRequestString = (Connect-OAuthTwitter $psTwitterEndpointFollowersIds $psTwitterParameters )
...
function Get-TwitterUserLookup
{
	<#
	.SYNOPSIS
		Sends a Twitter Tweet
	...
	BEGIN
	{
		(Write-Status -Message "START  - Get-UserLookupByIds function execution" -Status "INFO" -Debugging:$psTwitterDebugging -Logging:$psTwitterLogging -Logpath $psTwitterLogPath)
	}
	PROCESS
	{
		try
		{			
            ...
            $psTwitterMethod = ($psTwitterEndpointUserLookup -Split '&')[0]
            $psTwitterEndpoint = ($psTwitterEndpointUserLookup -Split '&')[1]

			$psTwitterParameters = "user_id=" + [System.Uri]::EscapeDataString($ids)
                
            # Call our main connect routine to do the oAuth heavy lifting
			$oAuthRequestString = (Connect-OAuthTwitter $psTwitterEndpointUserLookup $psTwitterParameters )
		...
		{
			Throw $("ERROR OCCURRED LOOKING UP USER " + $_.Exception.Message)
		}
	}
	END
	{
		(Write-Status -Message "FINISH - Get-UserLookupByIds function execution" -Status "INFO" -Debugging:$psTwitterDebugging -Logging:$psTwitterLogging -Logpath $psTwitterLogPath)
	}
}
	...
	PROCESS
	{
		...
		{			
			...
			$fixedTweetMessage = $(ConvertTo-HexEscaped -InputText $TweetMessage)
                ...
                $psTwitterMethod = ($psTwitterEndpointTweet -Split '&')[0]
                $psTwitterEndpoint = ($psTwitterEndpointTweet -Split '&')[1]
			    $psTwitterParameters = "status=$($fixedTweetMessage)"

            # Call our main connect routine to do the oAuth heavy lifting
			$oAuthRequestString = (Connect-OAuthTwitter $psTwitterEndpointTweet $psTwitterParameters )
			
			(Write-Status -Message "START  - Sending HTTP $psTwitterMethod via REST to Twitter" -Status "INFO" -Debugging:$psTwitterDebugging -Logging:$psTwitterLogging -Logpath $psTwitterLogPath)

			# Call the REST API to handle the final OAUTH POST
			Invoke-RestMethod -URI $psTwitterEndpoint -Method $psTwitterMethod -Body $psTwitterParameters -Headers @{ 'Authorization' = $oAuthRequestString } -ContentType "application/x-www-form-urlencoded" | Out-Null
	...
	PROCESS
	{
		try
		{
			...
			$fixedDirectMessage = $(ConvertTo-HexEscaped -InputText $DirectMessage)
                ...
                $psTwitterMethod = ($psTwitterEndpointDirectMessage -Split '&')[0]
                $psTwitterEndpoint = ($psTwitterEndpointDirectMessage -Split '&')[1]
			    $psTwitterParameters = "screen_name=$To&text=$($fixedDirectMessage)"

            # Call our main connect routine to do the oAuth heavy lifting
			$oAuthRequestString = (Connect-OAuthTwitter $psTwitterEndpointDirectMessage $psTwitterParameters )
